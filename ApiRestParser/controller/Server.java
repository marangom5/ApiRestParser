...
import BD.BD;
import USER.JPAUsers;
import USER.JPAUsersImpl;
...
public class Server {
    ...
    public Server(String method, String url, String headers, String body) {
        String request=method+" "+url+" "+versionProtocol+"\r\n"+headers+"\r\n"+body;
        if(!RequestParser.isValidRequest(request)&& !RequestParser.isValidHeader(request)){
            System.out.println("No es una request valida");
        }else{
            this.method = method;
            this.url = url;
        ...
        if(!RequestParser.isValidRequest(request)&& !RequestParser.isValidHeader(request)){
            ...
            switch (method) {
                ...
                case "DELETE":
                    if (url.equals("http://localhost:8080/api/users")) {
                        DeleteUserById(id);
                        BD.deleteDataById(String.valueOf(id));
                    }
                    break;
                case "GET":
                    if (url.equals("http://localhost:8080/api/users")) {
